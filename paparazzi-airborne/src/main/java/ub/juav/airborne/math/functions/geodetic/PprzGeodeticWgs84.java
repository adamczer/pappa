 ...
 */
public class PprzGeodeticWgs84 {
    private static int[][] pprz_geodetic_wgs84_int = {
    ...
    public static float WGS84_H(int x,int y){ return (float)pprz_geodetic_wgs84_int[(y)][(x)];}

    public static float wgs84_ellipsoid_to_geoid(float lat, float lon)
    {
        float x = (180.0f + UtilityFunctions.DegOfRad(lon)) / 10.0f;
        x = UtilityFunctions.Bound(x, 0.0f, 35.99999f);
        float y = (90.0f - UtilityFunctions.DegOfRad(lat)) / 10.0f;
        y = UtilityFunctions.Bound(y, 0.0f, 17.99999f);
        int ex1 = (int) x;
        int ex2 = ex1 + 1;
        if (ex2 >= 36) { ex2 = 0; }
        int ey1 = (int) y;
