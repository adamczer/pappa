...
//        GpsNative.gps_feed_value_finalize_juav();
        GpsNative.gps_feed_all_data_juav(
                1749,
                PaparazziNps.getNpsMainSimTime(),
                reading.getEcef_pos().getX(),reading.getEcef_pos().getY(),reading.getEcef_pos().getZ(),
                reading.getEcef_vel().getX(),reading.getEcef_vel().getY(),reading.getEcef_vel().getZ(),
                reading.getLla_pos().getLat(),reading.getLla_pos().getLon(),reading.getLla_pos().getAlt(),
                reading.getHmsl()
                );
    }
}
